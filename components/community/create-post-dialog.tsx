...
export function CreatePostDialog() {
  ...
  const onSubmit = async (data: PostForm) => {
    try {
      ...
      if (imageFile) {
        const uploadedUrl = await uploadFile(imageFile, "posts");
        if (uploadedUrl) {
        ...
        >
          ...
          <div className="space-y-2">
            ...
            <Input
              id="tags"
              {...register("tags")}
              placeholder="home, maintenance, DIY (comma separated)"
              className="w-full px-3 py-2 bg-white text-gray-900 placeholder:text-gray-400 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
            />
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              className="w-full px-3 py-2 bg-white text-gray-900 placeholder:text-gray-400 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
            />
            {errors.social_link_url && (
              <p className="text-sm text-red-600">
                {errors.social_link_url.message}
              </p>
            )}
          </div>
